  ...
  useEffect(() => {
    const fetchBillDetails = async () => {
      try {
        ...
        logger.error("Error fetching bill details:", err);
  ...
  useEffect(() => {
    const fetchBillAnalysis = async () => {
      try {
        ...
        logger.error("Error fetching bill analysis:", err);
        setAnalysis(null);
        setAnalysisLoading(false);
      }
    };
    ...
    <Container sx={{ mt: 4, mb: 4 }}>
      <Paper elevation={3} sx={{ p: 3, mb: 3 }}>
        <Stack spacing={2}>
          ...
          <Box>
            <Divider sx={{ my: 2 }} />
            <Typography variant="body1">{bill.description}</Typography>
          </Box>
        </Stack>
      </Paper>
      ...
      <Box sx={{ width: "100%" }}>
        ...
        <TabPanel value={tabValue} index={0}>
          <Paper
            elevation={1}
            sx={{ p: 3, maxHeight: "500px", overflow: "auto" }}
          >
            {bill.text ? (
              <Typography
                variant="body2"
            ...
            {bill.sponsors && bill.sponsors.length > 0 ? (
              <List>
                {bill.sponsors.map((sponsor, index) => (
                  <ListItem
                    key={`sponsor-${sponsor}-${index}`}
                    divider={index < bill.sponsors.length - 1}
                  >
            ...
            {bill.history && bill.history.length > 0 ? (
              <List>
                {bill.history.map((event, index) => (
                  <ListItem
                    key={`history-${event.date}-${index}`}
                    divider={index < bill.history.length - 1}
                  >
                    <ListItemText
            ...
            {bill.votes && bill.votes.length > 0 ? (
              <List>
                {bill.votes.map((vote, index) => (
                  <ListItem
                    key={`vote-${vote.date}-${vote.chamber}-${index}`}
                    divider={index < bill.votes.length - 1}
                  >
                    <ListItemText
        ...
        <TabPanel value={tabValue} index={4}>
          {analysisLoading ? (
            ...
            <Alert severity="info">No analysis available for this bill</Alert>
          )}
        </TabPanel>
      </Box>
    </Container>
